@echo off
REM This script configures ABCNote with the local MSVC, CMake, Ninja, and Qt toolchain.
REM It exists because PowerShell quoting can break paths that contain "Program Files (x86)".
...
REM BUILD_DIR is separate from earlier failed configure attempts.
...
REM Configure the project and explicitly pass rc.exe and mt.exe to avoid CMAKE_MT-NOTFOUND.
"%CMAKE_EXE%" -S "%PROJECT_DIR%" -B "%BUILD_DIR%" -G Ninja -DCMAKE_PREFIX_PATH="%QT_PREFIX%" -DCMAKE_BUILD_TYPE=Debug -DCMAKE_RC_COMPILER="%SDK_BIN%/rc.exe" -DCMAKE_MT="%SDK_BIN%/mt.exe"
exit /b %errorlevel%
